...
def main():
    ...
    train.to_csv("/content/drive/MyDrive/Masterarbeit/kFolden/train/train.csv")
    ...
    id_dev.to_csv("/content/drive/MyDrive/Masterarbeit/kFolden/dev/id_dev.csv")

    ood_dev = dev_dataset.filter(lambda example: example['intent'] not in label_ids)
    ood_dev = ood_dev.rename_column("text", "data")
    ood_dev.to_csv("/content/drive/MyDrive/Masterarbeit/kFolden/dev/ood_dev.csv")

    #Test
    id_test = dataset_dict['test_id_dataset'].rename_column("intent", "label")
    id_test = id_test.rename_column("text", "data")
    id_test.to_csv("/content/drive/MyDrive/Masterarbeit/kFolden/test/id_test.csv")

    ood_test = dataset_dict['test_ood_dataset'].rename_column("intent", "label")
    ood_test = ood_test.rename_column("text", "data")
    ood_test.to_csv("/content/drive/MyDrive/Masterarbeit/kFolden/test/ood_test.csv")
